...
deadset-go implements deadset contract version **2.1.0** and reads and writes report schema
...
**1.5.0**, which publishes nineteen fixtures, and the run answers the **17** fixtures of it that
...
At corpus version 1.5.0 this analyzer declares no gap, so `conformance.json` carries an empty list
