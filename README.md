...
- **Exemptions are computed, not configured.** A method that satisfies an interface a value of its type is converted to, a `main`, an `init`, a test function, a symbol a `go:linkname` directive names: each is held live by a documented exemption class, and `print-retained` lists every symbol an exemption held back and the class that held it.
